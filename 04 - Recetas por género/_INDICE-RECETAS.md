...
| [53](<./53 - Souls-like - lock-on, hoguera y pérdida de recursos al morir.md>) | **Souls-like: hoguera y pérdida de recursos al morir** | Alta | Checkpoint que reinicia enemigos y recursos consumibles del nivel (extiende `guardar_punto()` de `04 · 06`), marca de muerte recogible que desaparece si mueres una segunda vez, y el enganche con el lock-on de cámara de `13 · 19 §3.12` — sin repetir i-frames/parry (`04 · 30`) ni estamina (`04 · 36`), ya construidos | *Dark Souls*, *Sekiro*, *Hollow Knight* — cierra el bucle souls-like sobre `04 · 30`/`04 · 36` |
| [54](<./54 - Metajuego transversal - logros, galería, speedrun y espectador.md>) | **Metajuego transversal** | Media | Logros internos desacoplados de plataforma sobre `scr_save_load`, galería de desbloqueables leyendo `global.flags`, cronómetro de *splits* comparado en vivo contra un PB, y modo espectador (recibe estado, nunca envía input) sobre la base de red de `04 · 14 §10.1/§5.5/§5.6` | Técnica transversal — cualquier juego, publicado en una tienda con logros o no |
| [55](<./55 - Party games, minijuegos y creación casual.md>) | **Party games y creación casual** | Media-alta | Orquestador de minijuegos rotativos con contrato mínimo y catálogo como datos (reutiliza `04 · 11` para el contenido de cada microjuego), y secuenciador musical de creación (*step sequencer*) sobre el mismo reloj de `04 · 19` | *WarioWare* — construido sobre `04 · 11`/`04 · 19` |
| [56](<./56 - Combate no letal - pacifismo, aturdir, huir y negociar.md>) | **Combate no letal** | Media | Aturdir y noquear como tercera rama junto a "sigue con vida"/"muere" sobre el motor de efectos de `04 · 32`, huir de verdad cerrando la decisión que deja abierta el medidor de detección de `04 · 45`, negociar/sobornar reutilizando `Misiones` de `13 · 12`, intimidar como consideración de utility AI, dormir a distancia sin alertar al grupo, y el contador de "sin bajas" que hace que el mundo lo reconozca | *Dishonored*, *Undertale* — combate como opción, no como único desenlace del encuentro |
| [57](<./57 - Selección de nivel y capítulo.md>) | **Selección de nivel y capítulo** | Básica | El nivel como dato (id estable, requisitos de desbloqueo, resultado), «desbloqueado» derivado de «completado» y nunca guardado aparte, la rejilla con tres estados visuales, y la diferencia entre «Continuar» y «Selección de nivel» | Envoltura del juego — cualquier plataformas, puzzle o arcade con niveles discretos (hueco B3 cerrado, `_indice/auditorias/r5-juego-completo.md`) |
| [58](<./58 - El nivel como mapa de texto - construir sin abrir el editor de salas.md>) | **El nivel como mapa de texto** | Básica | El nivel en un array de cadenas y un constructor en el Create: validar antes de construir, saltarse la roca enterrada (30-65 % menos instancias), autotile de 4 bits y clave estable para el guardado. La única vía viable para un agente que no puede abrir el editor de salas | Cualquier juego 2D de rejilla — plataformas, puzles, mazmorras. Script listo en `06 · scr_nivel_mapa.gml` |
| [59](<./59 - Puertas, llaves y placas de presión - el puzle de sala.md>) | **Puertas, llaves y placas** | Media | Emisor → canal → receptor en vez de placa→puerta, la placa evaluada en el Step (si no, no hay «soltar»), abrir es `abierta = true` y nunca `instance_destroy`, llaves por clave estable, validar los canales al construir y reiniciar el nivel sin `room_restart()` | Puzles de sala, mazmorras, salas de escape — la otra mitad del Sokoban de `13 · 17` |
