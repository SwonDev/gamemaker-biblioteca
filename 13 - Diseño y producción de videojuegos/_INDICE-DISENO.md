...
> **El oficio que va antes y alrededor del código.** 26 documentos. Las otras carpetas explican el motor;
...
| [03](<./03 - Pixel art y resolución.md>) | **Pixel art y resolución** | Resolución base y escalado entero, paletas, clusters, dithering, contorno selectivo, animación en pixel art, Aseprite → GameMaker, y los ajustes del motor que emborronan un sprite (interpolación, texture pages, cámara a píxel entero) | eliges resolución, dibujas sprites o «todo se ve borroso» |
| [04](<./04 - Animación de sprites, Sequences y Animation Curves.md>) | **Animación** | Los 12 principios traducidos a sprites, `image_index`/`image_speed`, frame data (hit frames), crossfade entre sprites, animación procedural, cinemática inversa (IK de 2 huesos y CCD), **Animation Curves** y **Sequences** por código, timing en dobles | animas un personaje, montas una cinemática o una UI animada |
...
| [10](<./10 - Testing y QA.md>) | **Testing y QA** | Pirámide adaptada, GML testeable, mini-framework de pruebas que compila y corre con `gm-cli run --config`, GM-TestFramework y crispy, regresión y golden files, rendimiento, modo QA, **consola de comandos en runtime** (`keyboard_string`, historial, autocompletado, se apaga en Release), log con niveles y buffer sin coste por frame, plan de pruebas manual, informes de bug | antes de publicar, o cuando un bug vuelve por tercera vez, o quieres trastear el juego en marcha sin recompilar |
